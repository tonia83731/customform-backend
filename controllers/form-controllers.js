...
const formControllers = {
  ...
  publishedForm: async (req, res) => {
    try {
      ...
      if (isBecomingPublished)
        form.publishCount = (form.publishedCount || 0) + 1;
  ...
  addSection: async (req, res) => {
    try {
      ...
      const updated_form = await form.save();
      const updated_sections = updated_form.sections.sort(
        (a, b) => a.order - b.order
      );
  ...
  updatedSectionInfo: async (req, res) => {
    try {
  ...
  deleteSection: async (req, res) => {
    try {
      ...
      const delete_order = form.sections[sectionIdx].order;
      ...
      form.sections.forEach((sec) => {
        if (sec.order > delete_order) sec.order -= 1;
      });
  ...
  updatedQuestionsOrder: async (req, res) => {
    try {
      const { formId } = req.params;
